...
namespace area_L
{
    public partial class DANPLA_COLLECTOR_MATE : Form
    {
        ...
        private void fn_BarCode_Scan()
        {
            try
            {
                ...
                if (tbx_BarCode.Text.Trim().Length < 20)
                {
                    strSql = $"{dbName}.dbo.ST_DANPLA_COLLECTOR_BARCODE_SIMPLE_INFO_SEL";
                ...
                strSql = $"{dbName}.dbo.ST_DANPLA_COLLECTOR_BARCODE_DETAIL_SEL_FOR_MATE";
                ...
                if (db.nState)
                {
                    if (db.result.Rows.Count > 0)
                    {
                        ...
                        barcodeDataRow["EXPIR"] = dr["EXPIR"].ToString();
                        ...
                        //if (dr["MATE_NO"].ToString().Equals("-"))
                        //{
                        //    INSERT_MATENO_FORDANPLA imf = new INSERT_MATENO_FORDANPLA(this);

                        //    imf.ShowDialog();

                        //    barcodeDataRow["LOT_NO"] = tbx_LotNo.Text;

                        //    barcodeDataRow["MATE_NO"] = tbx_result_MATE_NO.Text;
                        //}
                        //else
                        //{
                        ...
                        if (dr["CHUL_GU_BUNRYU"].ToString().Equals("N"))
                        {
                            ...
                            new CustomMessageBox(tbx_CHULGO.Text, "스캔").ShowDialog();

                            tbx_BarCode.SelectAll();
                        }

                        if(dr["SCANORNOT"].ToString().Equals("O") && dr["CHUL_GU_BUNRYU"].ToString().Equals("Y"))
                        {

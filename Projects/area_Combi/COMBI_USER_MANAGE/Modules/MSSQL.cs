...
namespace ERP_IF_PRO.Modules
{
    class MSSQL
    {
        ...
        public Dictionary<string, string> OutputParameterMapper = new Dictionary<string, string>();
        ...
        public bool ExecuteNonSql(string Sp_name)
        {
            try
            {
                ...
                using (conn = new SqlConnection(connectionStr))
                {
                    ...
                    if (OutputParameterMapper.Count() > 0)
                    {
                        foreach (SqlParameter sp in outputparameters)
                        {
                            cmd.Parameters.Add(sp);
                        }
                    }
                    ...
                    foreach (SqlParameter sp in outputparameters)
                    {
                        if (OutputParameterMapper.ContainsKey(sp.ParameterName)) OutputParameterMapper[sp.ParameterName] = sp.Value.ToString();
                    }
                }
        ...
        public void OutputParameter(string mapStr)
        {
            try
            {
                if(!this.OutputParameterMapper.ContainsKey(mapStr)) this.OutputParameterMapper.Add(mapStr, string.Empty);

                SqlParameter sp = new SqlParameter(mapStr, SqlDbType.VarChar ,500);

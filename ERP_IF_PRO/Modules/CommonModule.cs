...
namespace ERP_IF_PRO.Modules
{
    ...
    class CommonModule
    {
        ...
        public void ComboBoxBinding(System.Data.DataTable dtTbl, System.Windows.Forms.ComboBox combo, string code, Dictionary<string,string> dic)
        {
            try
            {
                DataRow[] dr = dtTbl.Select($"KIND = '{code}'");
                ...
                foreach (DataRow ddr in dr)
                {
                    ...
                    combo.Items.Add(spliters[0]);

                    dic.Add(spliters[0], spliters[1]);
                }
            }
            catch (Exception ex)
            {
        ...
        public void writeLog(string dbName,string GUBUN, string frmnm, string funcnm,string ip,string err_msg, string ps_cd)
        {
            try
            {
                string strSql = $"{dbName}.dbo.ST_EMAX_IF_LOG_UPD";
        ...
        public bool LoadForm(TabControl tabControl, string formName, string menuName)
        {
            try
            {
                ...
                var frm = (Form)Activator.CreateInstance(Type.GetType($"COMBINATION.{formName}"));
        ...
        public bool LoadForm(TabControl tabControl, System.Windows.Forms.Label lb_Status, System.Windows.Forms.Label lb_Status2, string formName, string menuName, bool DockOrNot, bool isMaximize, float menuHeight)
        {
            try
            {
                foreach (TabPage tb in tabControl.TabPages)
                ...
                var frm = (Form)Activator.CreateInstance(Type.GetType($"COMBINATION.{formName}"), lb_Status, lb_Status2);
